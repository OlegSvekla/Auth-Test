...
/// <summary>
/// Code created by support https://github.com/aspnet/Identity/blob/3fc7fec4a225eaafb5021aaeed926143b2cb6892/src/Core/EmailTokenProvider.cs
/// </summary>
/// <typeparam name="TUser"></typeparam>
public class FourDigitEmailTokenProvider<TUser>
    ...
    where TUser : EfIdentityUserEntity
{
        ...
        TUser user)
    {
        ...
        var securityToken = await manager.CreateSecurityTokenAsync(user);
        var otpToken = new OtpSecurityToken(securityToken);
        var modifier = await GetUserModifierAsync(purpose, manager, user);

        var valid = Rfc6238AuthenticationService
            .ValidateCode(otpToken, code, modifier, token.Length);
        return valid;
    }
}
